...
<div id="minorModal" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center hidden z-50">
    <div class="bg-white rounded p-6 w-full max-w-md shadow-lg relative">
        ...
        <form id="minorForm" action="<?= base_url('waiver/children/add') ?>" method="post" class="space-y-4">
            <?= csrf_field() ?>
...
<script>
    function openModal(isEdit = false, child = null) {
        ...
        if (isEdit && child) {
            ...
            document.getElementById('birth_year').value = birth.getFullYear();
            document.getElementById('birth_month_children').value = birth.getMonth() + 1;
            document.getElementById('birth_day').value = birth.getDate();

            // Month selected
            $('#birth_month_children').val(birth.getMonth() + 1).trigger('change');

            if (child.gender === 'male' || child.gender === 'female') {
                const genderEl = document.getElementById('gender-' + child.gender);
    ...
    function editChild(id) {
        fetch('<?= base_url('waiver/children/get/') ?>' + id)
            .then(res => {
                if (!res.ok) throw new Error('Network response was not ok');
                return res.json();
            })
            .then(child => openModal(true, child))
            .catch(err => alert('Failed to fetch data: ' + err.message));
    }
