...
<body class="bg-gray-50 min-h-screen">
        ...
        <aside id="sidebar" class="w-full md:w-64 bg-white shadow-md px-4 py-6 hidden md:block fixed md:static top-0 left-0 h-full z-50 md:h-auto md:relative">
            ...
            <nav class="space-y-2">
                ...
                <a href="<?= base_url('admin/change-password') ?>"
                    ...
                    class="block py-2 px-4 rounded hover:bg-primary hover:text-white <?= url_is('admin/change-password') ? 'bg-primary text-white' : 'text-gray-700' ?>">
                    Change Password
                </a>
    ...
    <script>
        document.getElementById('changePasswordLink')?.addEventListener('click', function(e) {
            e.preventDefault(); // cegah default redirect
            ...
            Swal.fire({
                ...
                confirmButtonColor: '#016BAF',
                cancelButtonColor: '#d33',
                confirmButtonText: 'Yes, change it!',
                cancelButtonText: 'Cancel'
            }).then((result) => {
                if (result.isConfirmed) {
                    window.location.href = this.href; // redirect ke change password
                }
            });
        });
