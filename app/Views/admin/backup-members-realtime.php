...
<div class="bg-white shadow rounded-lg overflow-hidden">
    ...
    <div class="w-full overflow-auto">
        <form method="get" class="px-4 sm:px-6 py-4 border-b flex flex-wrap items-center gap-3">
            <input type="text" name="keyword" id="searchInput" value="<?= esc($keyword ?? '') ?>"
                placeholder="Cari nama, email, atau negara..."
                class="border border-gray-300 rounded px-3 py-2 text-sm w-full sm:w-64" />
            <select name="country" id="filterCountry"
                class="border border-gray-300 rounded px-3 py-2 text-sm w-full sm:w-48">
                <option value="">Filter Negara</option>
                <?php foreach ($countries as $c): ?>
                <option value="<?= esc($c) ?>" <?=($c==($selectedCountry ?? '' )) ? 'selected' : '' ?>>
                    <?= esc($c) ?>
                </option>
                <?php endforeach; ?>
            </select>
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 sm:px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center gap-2">
                            ...
                            <img src="https://flagcdn.com/w20/<?= esc($m['country_code']) ?>.png"
                                alt="<?= esc($m['country']) ?>" class="inline-block w-5 h-auto rounded-sm">
                            <?php endif; ?>
                            <span>
                                <?= esc($m['country']) ?>
                            </span>
                    ...
                    <td class="px-4 sm:px-6 py-4 whitespace-nowrap flex gap-2">
                        ...
                        <button data-delete-url="/admin/member/delete/<?= esc($m['id']) ?>"
...
<script>
    let lastNotifCountMembers = 0;
    let lastMemberIds = [];

    async function checkNewNotificationsAndUpdate() {
        try {
            const res = await fetch("<?= base_url('admin/notifications') ?>");
            const notifs = await res.json();
            const unread = notifs.filter(n => n.is_read == 0);

            if (unread.length > lastNotifCountMembers) {
                console.log("🔁 New notification detected. Refreshing members...");
                fetchMembers();
            }

            lastNotifCountMembers = unread.length;
        } catch (err) {
    ...
    async function fetchMembers() {
        ...
        try {
            const res = await fetch(`<?= base_url('admin/member/search') ?>?q=${encodeURIComponent(q)}&country=${encodeURIComponent(country)}`);
            const data = await res.json();

            const newIds = data.map(m => m.id);
            if (JSON.stringify(newIds) === JSON.stringify(lastMemberIds)) return;
            lastMemberIds = newIds;
    ...
    setInterval(checkNewNotificationsAndUpdate, 10000);
